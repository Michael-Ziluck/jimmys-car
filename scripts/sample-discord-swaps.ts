...
async function main(): Promise<void> {
  ...
  for (let index: number = 0; index < 5; index += 1) {
    ...
    const matches: DiscordMessage[] = slice.filter((message) =>
      candidates.includes(message),
    );
    console.log(
    ...
    for (const message of matches.slice(0, 12)) {
      ...
      const spotifyLinks: [] | RegExpMatchArray =
        text.match(SPOTIFY_URLS) ?? [];
      console.log(
        JSON.stringify({
          id: message.id,
