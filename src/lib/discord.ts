...
export type DiscordProfile = { id: string; username: string; global_name: string | null; avatar: string | null };
...
function getDiscordConfig(): DiscordConfig {
  ...
  if (!DISCORD_CLIENT_ID || !DISCORD_CLIENT_SECRET || !DISCORD_REDIRECT_URI) {
    throw new Error("Discord is not configured. Add the Discord values to .env.local.");
  }
  return { clientId: DISCORD_CLIENT_ID, clientSecret: DISCORD_CLIENT_SECRET, redirectUri: DISCORD_REDIRECT_URI };
}
...
export async function getDiscordProfile(accessToken: string): Promise<DiscordProfile> {
  const response: Response = await fetch(`${DISCORD_API}/users/@me`, { headers: { Authorization: `Bearer ${accessToken}` }, cache: "no-store" });
...
export function discordAvatarUrl(profile: DiscordProfile): string | null {
  return profile.avatar ? `https://cdn.discordapp.com/avatars/${profile.id}/${profile.avatar}.png?size=128` : null;
}
