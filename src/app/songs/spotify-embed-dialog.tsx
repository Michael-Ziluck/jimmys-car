...
export function SpotifyEmbedDialog({ trackId, songTitle }: { trackId: string; songTitle: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/spotify/oembed?trackId=${encodeURIComponent(trackId)}`, { signal: controller.signal })
      ...
      .catch((reason: unknown) => {
        ...
        setError(reason instanceof Error ? reason.message : "Spotify could not load this player.");
      });
  ...
  useEffect(() => {
    ...
    loadSpotifyIframeApi()
      ...
      .catch((reason: unknown) => {
        if (!cancelled) {
          setError(reason instanceof Error ? reason.message : "Spotify could not load its player API.");
        }
      });
      ...
      >
        <div className="grid gap-4 p-4">
          <div className="flex flex-col gap-2 pr-8">
            <h2 id={`spotify-player-${trackId}`} className="font-heading text-base leading-none font-medium">
              {embed?.title ?? songTitle}
            </h2>
            <p className="text-sm text-muted-foreground">Spotify player</p>
          </div>
          {hasOpened && (embed ? (
            <div ref={playerElementRef} className="min-h-38 w-full overflow-hidden rounded-xl" />
          ) : error ? (
            <p className="rounded-lg bg-destructive/10 p-3 text-sm text-destructive">{error}</p>
          ) : (
            <div className="h-38 animate-pulse rounded-xl bg-muted" aria-label="Loading Spotify player" />
          ))}
          <div className="-mx-4 -mb-4 flex flex-col-reverse gap-2 rounded-b-xl border-t bg-muted/50 p-4 sm:flex-row sm:justify-end">
          <Button asChild variant="outline">
            <a href={embed?.spotifyUrl ?? `https://open.spotify.com/track/${trackId}`} target="_blank" rel="noreferrer">
              Open in Spotify
