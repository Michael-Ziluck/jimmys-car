...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  const requestOrigin: string = request.headers.get("host")
    ? `${requestUrl.protocol}//${request.headers.get("host")}`
    : requestUrl.origin;
  if (requestOrigin !== configuredOrigin) {
    const normalizedUrl: URL = new URL(
  ...
  const response: NextResponse = NextResponse.redirect(
    createDiscordAuthorizationUrl(state),
  );
  response.cookies.set(discordStateCookieName, state, {
    httpOnly: true,
    maxAge: 10 * 60,
    sameSite: "lax",
